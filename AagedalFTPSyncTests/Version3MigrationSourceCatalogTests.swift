...
final class Version3MigrationSourceCatalogTests: XCTestCase {
    ...
    func testEmptyInventoryOffersOnlyExplicitAbsenceAndBuildsCompleteFrozenPlanWithoutWrites() throws {
        ...
        XCTAssertFalse(catalog.hasLegacyData)
        XCTAssertTrue(catalog.supportsStreamlinedUpgrade)
        XCTAssertEqual(catalog.recommendedPrimarySources, selections(catalog))
        XCTAssertEqual(catalog.recommendedSignatureSource, .absent)
    ...
    func testDamagedPrimaryAndExistingBackupRemainDistinctChoicesWithNoImplicitFallback() throws {
        ...
        let catalog = try Catalog.inspect(root: root)
        XCTAssertTrue(catalog.hasLegacyData)
        ...
        XCTAssertEqual(catalog.recommendedPrimarySources[names[0]], .file(names[0]))
        XCTAssertNil(catalog.recommendedPrimarySources[names[1]])
        XCTAssertFalse(catalog.supportsStreamlinedUpgrade)
    ...
    func testAllKnownSignatureVariantsRemainExplicitFormatsAndNeverOfferAbsent() throws {
        ...
        XCTAssertEqual(catalog.recommendedSignatureSource, .sqlite(main))
        XCTAssertTrue(catalog.supportsStreamlinedUpgrade)
