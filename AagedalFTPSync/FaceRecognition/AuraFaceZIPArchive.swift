...
final class AuraFaceZIPArchive {
    ...
    init(url: URL, descriptor: AuraFaceDistributionDescriptor) throws {
        ...
        guard fstat(fd, &identity) == 0, identity.st_mode & S_IFMT == S_IFREG,
              identity.st_nlink == 1, identity.st_size == descriptor.archive.byteCount else {
            throw AuraFaceComponentError.unsafeArchive
        }
        let size = Int(identity.st_size)
        guard size >= 22, size <= AuraFaceDistributionContract.maximumArchiveBytes else {
        ...
        for _ in 0..<3 {
            ...
            guard let path = String(data: name, encoding: .utf8),
                  path.hasPrefix(AuraFaceDistributionContract.packageDirectory + "/") else {
                throw AuraFaceComponentError.unsafeArchive
            }
            let relative = String(path.dropFirst(AuraFaceDistributionContract.packageDirectory.count + 1))
            ...
            guard collisionKeys.insert(collision).inserted,
                  let declaration = descriptor.packageFiles[relative],
                  declaration.byteCount == Int(uncompressed) else {
        ...
        guard cursor == central.count, Set(parsed.map(\.relativePath)) == AuraFaceDistributionContract.packagePaths else {
            throw AuraFaceComponentError.packageFileSetMismatch
        }
    ...
    func extract(to package: URL, descriptor: AuraFaceDistributionDescriptor) throws {
        defer { close(fd); fd = -1 }
        try FileManager.default.createDirectory(at: package, withIntermediateDirectories: false,
                                                attributes: [.posixPermissions: 0o700])
        for entry in entries.sorted(by: { $0.relativePath < $1.relativePath }) {
            try Task.checkCancellation()
            let destination = package.appendingPathComponent(entry.relativePath)
            ...
            var succeeded = false
            defer { close(output); if !succeeded { _ = unlink(destination.path) } }
            ...
            guard fsync(output) == 0 else { throw AuraFaceComponentError.io }
            succeeded = true
        }
        var after = stat()
    ...
    private static func validate(_ path: String) throws {
        ...
        guard !path.isEmpty, path.utf8.count <= 1_024, !path.hasPrefix("/"), !path.contains("\\"),
              ...
              AuraFaceDistributionContract.packagePaths.contains(path) else {
            throw AuraFaceComponentError.unsafeArchive
        }
    }
