...
/// FTP-side People Library interchange contract.
/// These types perform no extraction, filesystem access, or snapshot publication.
struct PeopleLibraryManifest: Codable, Equatable, Sendable {
    ...
    struct EmbeddingContract: Codable, Equatable, Sendable {
        ...
        init(from decoder: Decoder) throws {
            let c = try PeopleLibraryCoding.object(decoder, required: ["embeddingSpaceVersion", "componentID", "modelID", "preprocessingRevision", "vectorEncoding", "dimension", "l2Normalized"])
            self.init()
    ...
    struct Exporter: Codable, Equatable, Sendable {
        ...
        init(app: String, version: String, sourceRevision: String) throws {
            guard PeopleLibraryCoding.text(app, maximumBytes: 128), PeopleLibraryCoding.text(version, maximumBytes: 128),
                  sourceRevision.utf8.count == 40, PeopleLibraryCoding.lowerHex(sourceRevision) else { throw ValidationError.invalidIdentity }
            self.app = app; self.version = version; self.sourceRevision = sourceRevision
        }
        init(from decoder: Decoder) throws {
            let c = try PeopleLibraryCoding.object(decoder, required: ["app", "version", "sourceRevision"])
    ...
    struct FileDeclaration: Codable, Equatable, Sendable {
        ...
        init(path: String, byteCount: Int, sha256: String) throws {
            try PeopleLibraryCoding.validatePath(path)
            guard byteCount > 0, byteCount <= Limits().maximumFileBytes else { throw ValidationError.exceededLimit }
            guard sha256.utf8.count == 64, PeopleLibraryCoding.lowerHex(sha256) else { throw ValidationError.invalidHash }
            if path.hasPrefix("embeddings/") {
                guard byteCount == FaceRecognitionEmbeddingCodec.encodedByteCount else { throw ValidationError.invalidPayload }
            }
            if path.hasPrefix("upgrade_sources/") {
                guard byteCount <= 1_000_000 else { throw ValidationError.exceededLimit }
            }
            self.path = path; self.byteCount = byteCount; self.sha256 = sha256
        }
        init(from decoder: Decoder) throws {
            let c = try PeopleLibraryCoding.object(decoder, required: ["path", "byteCount", "sha256"])
    ...
    static func decode(_ data: Data, limits: Limits = .init()) throws -> Self {
        ...
        try PeopleLibraryCoding.validateJSON(data)
    ...
    func validate(limits: Limits = .init()) throws {
        ...
        guard format == Self.formatIdentifier, [2, 3].contains(schemaVersion),
              schemaVersion == 3 || !files.contains(where: { $0.path.hasPrefix("upgrade_sources/") }) else {
            throw ValidationError.invalidSchema
        }
        try PeopleLibraryCoding.validateID(libraryID)
        guard revision.utf8.count == 64, PeopleLibraryCoding.lowerHex(revision) else { throw ValidationError.invalidHash }
        ...
        for file in files {
            try PeopleLibraryCoding.validatePath(file.path)
            ...
            if file.path.hasPrefix("upgrade_sources/"), file.byteCount > 1_000_000 { throw ValidationError.exceededLimit }
    ...
    func validate(payload: PeopleLibraryPayload, limits: Limits = .init()) throws {
        ...
        for person in payload.people {
            ...
            for example in person.examples {
                ...
                if let path = example.upgradeSourcePath {
                    guard schemaVersion == 3 else { throw ValidationError.invalidSchema }
                    guard referenced.insert(path).inserted else { throw ValidationError.duplicatePath }
                }
            }
        }
...
struct PeopleLibraryPayload: Codable, Equatable, Sendable {
    struct Example: Codable, Equatable, Sendable {
        let id: UUID
        let embeddingPath: String
        let thumbnailPath: String?
        let upgradeSourcePath: String?
        init(id: UUID, embeddingPath: String, thumbnailPath: String? = nil, upgradeSourcePath: String? = nil) throws {
            try PeopleLibraryCoding.validateID(id)
            guard embeddingPath == "embeddings/\(id.uuidString.lowercased()).fem2",
                ...
                throw PeopleLibraryManifest.ValidationError.invalidReference
            }
            self.id = id; self.embeddingPath = embeddingPath; self.thumbnailPath = thumbnailPath
            self.upgradeSourcePath = upgradeSourcePath
        }
        init(from decoder: Decoder) throws {
            let c = try PeopleLibraryCoding.object(decoder, required: ["id", "embeddingPath"], optional: ["thumbnailPath", "upgradeSourcePath"])
            try self.init(id: PeopleLibraryCoding.id(from: c, key: "id"), embeddingPath: c.decode(String.self, forKey: .init("embeddingPath")),
    ...
    struct Person: Codable, Equatable, Sendable {
        ...
        init(id: UUID, name: String, examples: [Example], thumbnailPath: String? = nil) throws {
            try PeopleLibraryCoding.validateID(id)
            guard PeopleLibraryCoding.text(name, maximumBytes: PeopleLibraryManifest.Limits().maximumNameUTF8Bytes), !examples.isEmpty,
                  examples.count <= PeopleLibraryManifest.Limits().maximumEmbeddings,
                  thumbnailPath == nil || thumbnailPath == "thumbnails/\(id.uuidString.lowercased()).jpg" else { throw PeopleLibraryManifest.ValidationError.invalidPayload }
            self.id = id; self.name = name; self.examples = examples; self.thumbnailPath = thumbnailPath
        }
        init(from decoder: Decoder) throws {
            let c = try PeopleLibraryCoding.object(decoder, required: ["id", "name", "examples"], optional: ["thumbnailPath"])
            try self.init(id: PeopleLibraryCoding.id(from: c, key: "id"), name: c.decode(String.self, forKey: .init("name")),
                examples: PeopleLibraryCoding.array(Example.self, from: c, key: "examples", maximum: PeopleLibraryManifest.Limits().maximumEmbeddings),
                thumbnailPath: c.contains(.init("thumbnailPath")) ? c.decode(String.self, forKey: .init("thumbnailPath")) : nil)
        }
        func encode(to encoder: Encoder) throws {
            var c = encoder.container(keyedBy: PeopleLibraryCoding.Key.self)
    ...
    static func decode(_ data: Data, limits: PeopleLibraryManifest.Limits = .init()) throws -> Self {
        try limits.validate()
        guard data.count <= limits.maximumPayloadBytes else { throw PeopleLibraryManifest.ValidationError.exceededLimit }
        try PeopleLibraryCoding.validateJSON(data)
        let value = try JSONDecoder().decode(Self.self, from: data); try value.validate(limits: limits); return value
    }
    func validate(limits: PeopleLibraryManifest.Limits = .init()) throws {
        try limits.validate()
        guard people.count <= limits.maximumPeople else { throw PeopleLibraryManifest.ValidationError.exceededLimit }
        var ids: Set<UUID> = [], examples: Set<UUID> = []
        for person in people {
            guard ids.insert(person.id).inserted else { throw PeopleLibraryManifest.ValidationError.duplicateIdentity }
            guard person.name.utf8.count <= limits.maximumNameUTF8Bytes else { throw PeopleLibraryManifest.ValidationError.exceededLimit }
            for example in person.examples {
                guard examples.count < limits.maximumEmbeddings else { throw PeopleLibraryManifest.ValidationError.exceededLimit }
                guard examples.insert(example.id).inserted else { throw PeopleLibraryManifest.ValidationError.duplicateIdentity }
            }
        }
    }
    init(from decoder: Decoder) throws {
        let c = try PeopleLibraryCoding.object(decoder, required: ["people"])
        people = try PeopleLibraryCoding.array(Person.self, from: c, key: "people", maximum: PeopleLibraryManifest.Limits().maximumPeople)
        try validate()
    }
}

private enum PeopleLibraryCoding {
    ...
    static func validateJSON(_ data: Data) throws {
        ...
        scanner.whitespace()
        guard scanner.index == scanner.bytes.count else { throw PeopleLibraryManifest.ValidationError.invalidPayload }
    }
    private struct JSONKeys {
        ...
        mutating func consume(_ byte: UInt8) throws {
            whitespace()
            guard index < bytes.count, bytes[index] == byte else { throw PeopleLibraryManifest.ValidationError.invalidPayload }
            index += 1
        }
        mutating func string() throws -> Range<Int> {
            ...
            while index < bytes.count {
                ...
                guard byte >= 32 else { throw PeopleLibraryManifest.ValidationError.invalidPayload }
                if byte == 92 {
                    guard index < bytes.count else { throw PeopleLibraryManifest.ValidationError.invalidPayload }
                    index += 1
                }
            }
            throw PeopleLibraryManifest.ValidationError.invalidPayload
        }
        mutating func value(depth: Int) throws {
            guard depth < 64 else { throw PeopleLibraryManifest.ValidationError.exceededLimit }
            whitespace()
            guard index < bytes.count else { throw PeopleLibraryManifest.ValidationError.invalidPayload }
            switch bytes[index] {
            case 123:
                ...
                while true {
                    ...
                    guard keys.insert(key).inserted else { throw PeopleLibraryManifest.ValidationError.invalidSchema }
            ...
            default:
                ...
                guard index > start else { throw PeopleLibraryManifest.ValidationError.invalidPayload }
            }
        }
    }
    ...
    static func object(_ decoder: Decoder, required: Set<String>, optional: Set<String> = []) throws -> KeyedDecodingContainer<Key> {
        ...
        guard required.isSubset(of: keys), keys.isSubset(of: required.union(optional)) else { throw PeopleLibraryManifest.ValidationError.invalidSchema }
        return c
    }
    static func array<T: Decodable>(_ type: T.Type, from c: KeyedDecodingContainer<Key>, key: String, maximum: Int) throws -> [T] {
        ...
        if let count = values.count, count > maximum { throw PeopleLibraryManifest.ValidationError.exceededLimit }
        while !values.isAtEnd {
            guard result.count < maximum else { throw PeopleLibraryManifest.ValidationError.exceededLimit }
    ...
    static func id(from c: KeyedDecodingContainer<Key>, key: String) throws -> UUID {
        ...
        guard let id = UUID(uuidString: raw), raw == id.uuidString.lowercased() else { throw PeopleLibraryManifest.ValidationError.invalidIdentity }
        try validateID(id)
        return id
    }
    ...
    static func validatePath(_ path: String) throws {
        ...
        default: throw PeopleLibraryManifest.ValidationError.invalidPath
        }
        guard name.hasSuffix(suffix), let id = UUID(uuidString: String(name.dropLast(suffix.count))),
              name == id.uuidString.lowercased() + suffix else { throw PeopleLibraryManifest.ValidationError.invalidPath }
        try validateID(id)
    }
}
