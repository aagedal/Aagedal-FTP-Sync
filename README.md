...
Calendar sharing uses a user-configured HTTPS PHP/MySQL server: whole calendars or selected dates, editor/read-only invitations, offline edits and explicit conflict resolution. Configure it under **Settings → Metadata Sync**. FTP credentials and local processing policies remain private to each Mac. Choose [Docker deployment](Server/MetadataSync/DOCKER.md) or [manual PHP/SQL setup](Server/MetadataSync/MANUAL-SETUP.md). See the [server reference](Server/MetadataSync/README.md) for date-range boundaries, limits and deployment verification.

In Calendar Sync, choose **Connect job to server**, select the **Sync Job**, and paste the complete join string. Its server URL is read automatically and a single matching calendar is linked directly; existing programming is preserved through the copy review. **Initialize Sync server** is for the administrator connecting the first Mac. New connections and calendars use 3.0 template-enabled sync. Use **Members & Invitations** to create invitations, limit an invitation's date range, and manage access. Invitations include the server URL by default. Several jobs can use one server, and jobs on different servers continue syncing together. Each job has its own calendar link; a shared calendar can be linked to one job per Mac. Selecting a linked job shows its server automatically.
