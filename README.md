...
Calendar sharing uses a user-configured HTTPS PHP/MySQL server: whole calendars or selected dates, editor/read-only invitations, offline edits and explicit conflict resolution. Manage servers under **Settings → Sync Servers**, and attach calendars under **Job settings → Metadata Sync**. FTP credentials and local processing policies remain private to each Mac. Choose [Docker deployment](Server/MetadataSync/DOCKER.md) or [manual PHP/SQL setup](Server/MetadataSync/MANUAL-SETUP.md). See the [server reference](Server/MetadataSync/README.md) for date-range boundaries, limits and deployment verification.
