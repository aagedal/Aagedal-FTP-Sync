...
def main():
    ...
    for phase in ("prepared", "originalsHeld", "published-0", "beforeCommit"):
        fixture = output / f"aagedal-interruption-{phase}"
        fixture.mkdir()
        ...
        try:
            for role, test in (("worker", "testProcessInterruptionWorker"), ("recovery", "testProcessInterruptionRecovery")):
                command = ["xcodebuild", "test-without-building", "-xctestrun", str(configured),
                           ...
                           "-resultBundlePath", str(output / f"{phase}-{role}.xcresult")]
                with (output / f"{phase}-{role}.log").open("w") as log:
                ...
                if role == "worker":
                    summary = json.loads(subprocess.check_output([
                        ...
                        str(output / f"{phase}-{role}.xcresult"), "--format", "json"], timeout=30))
                    ...
                    if not killed:
                        raise RuntimeError(f"{phase}: worker did not report SIGKILL: {failures}")
                    if result.returncode != 65 or not reached:
                        raise RuntimeError(f"{phase}: expected killed worker (exit 65) with exact phase marker, got {result.returncode}")
                elif result.returncode != 0:
                    raise RuntimeError(f"{phase}: fresh-process recovery failed ({result.returncode})")
                if role == "recovery":
                    verified = fixture / "inputs" / "recovery-verified"
                    if not verified.is_file() or verified.read_text() != phase:
                        raise RuntimeError(f"{phase}: recovery test did not reach its final assertion")
                results.append({"phase": phase, "role": role, "exitCode": result.returncode})
                print(f"{phase} {role}: expected exit {result.returncode}", flush=True)
                (output / "results.json").write_text(json.dumps(results, indent=2) + "\n")
        finally:
            configured.unlink()
    print("PASS: all four interruptions recovered and admitted a fresh publication", flush=True)
