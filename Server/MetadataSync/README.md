...
Independent edits to different clips or different fields of one clip merge automatically. When both Macs change the same value, **Resolve Conflicts…** offers a choice for each affected clip while retaining independent changes from both versions. Deleting a clip while someone edits it also requires a choice. Overlapping schedules are reviewed as an affected group; choosing its schedule preserves independently merged text and locations. Photographer and day-row conflicts are separate items. If either calendar changes during review, refresh the review before applying choices. Read-only members must accept server values for local changes.
